...
# with regard to varying incoming solar radiation
...
sol_list_sel = [1200,1300,1340,1400]  # selected sols

# monthly average of last 20 years
# adapt the directory to connect to the folder sol_20years_ymonmean
data_dir = 'C:/Users/anjaka/Nextcloud/PhD/03_MonsoonPlanet_Hysteresis/data/sensitivity/'

# Initialize dictionaries to store the data for each sol
ds_dict = {}
...
for value in sol_list_sel:
    # Load the dataset
    ds_dict[value] = xr.open_dataset(f'{data_dir}MonsoonPlanet_sol_{value}.nc')
...
# %%
### PLOT: HYSTERESIS SOLAR RADIATION
#-------------------------------------
...
# Define colors for each sol radiation
colors = ["#DC143C", "#FF8000", "#33A1C9", "#191970"]

# Plot 1
for i, sol in enumerate(sol_list_sel):
    sw = sw_m_z_mean[sol]
    wvp = wvp_m_z_mean[sol]
    axs[1, 0].plot(sw, wvp, label=f'{sol}', color=colors[i])
...
for i, sol in enumerate(sol_list_sel):
    sw = sw_m_z_mean[sol]
    pr = pr_m_z_mean[sol]
    axs[0, 0].plot(sw, pr, label=f'{sol}', color=colors[i])
...
# Plot 3
for i, sol in enumerate(sol_list_sel):
    tsurf = tsurf_m_z_mean[sol]
    pr = pr_m_z_mean[sol]
    axs[0, 1].plot(tsurf, pr, label=f'{sol}', color=colors[i])
...
for i, sol in enumerate(sol_list_sel):
    tsurf = tsurf_m_z_mean[sol]
    wvp = wvp_m_z_mean[sol]
    axs[1, 1].plot(tsurf, wvp, label=f'{sol}', color=colors[i])
...
plt.legend(loc='upper left', bbox_to_anchor=(1.05, 0.5), fancybox=True, shadow=True, ncol=1, title="Solar Radiation ($W/m^2$)",
...
plt.savefig(save_dir + '/robustness/MonsoonPlanet_Hysteresis_4panels_sol.pdf', bbox_inches='tight')
