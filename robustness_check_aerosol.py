...
# with regard to varying atmospheric aerosol concentration
...
aerosol_list_sel = ['0','6','5','4']  
aerosol_list_sel2 = ['0','1','10','100']

# monthly average of last 20 years
# adapt the directory to connect to the folder aerosol_20years_ymonmean
...
for value in aerosol_list_sel:
    # Load the dataset
    ds_dict[value] = xr.open_dataset(f'{data_dir}MonsoonPlanet_aerosol_{value}.nc')
...
# %%
# PLOT: ROBUSTNESS AEROSOL
#---------------------------
plt.rcParams.update({'font.size': 14})

fig, axs = plt.subplots(2, 2, figsize=(10, 8))

# Define colors for each aerosol depth
colors = ["#DC143C", "#FF8000", "#33A1C9", "#191970"]

# Plot 1
for i, aerosol in enumerate(aerosol_list_sel):
    sw = sw_m_z_mean[aerosol]
    wvp = wvp_m_z_mean[aerosol]
    axs[1, 0].plot(sw, wvp, label=f'{aerosol_list_sel2[i]}', color=colors[i])
...
for i, aerosol in enumerate(aerosol_list_sel):
    sw = sw_m_z_mean[aerosol]
    pr = pr_m_z_mean[aerosol]
    axs[0, 0].plot(sw, pr, label=f'{aerosol_list_sel2[i]}', color=colors[i])
...
# Plot 3
for i, aerosol in enumerate(aerosol_list_sel):
    tsurf = tsurf_m_z_mean[aerosol]
    pr = pr_m_z_mean[aerosol]
    axs[0, 1].plot(tsurf, pr, label=f'{aerosol_list_sel2[i]}', color=colors[i])
...
for i, aerosol in enumerate(aerosol_list_sel):
    tsurf = tsurf_m_z_mean[aerosol]
    wvp = wvp_m_z_mean[aerosol]
    axs[1, 1].plot(tsurf, wvp, label=f'{aerosol_list_sel2[i]}', color=colors[i])
...
plt.legend(loc='upper left', bbox_to_anchor=(1.05, 0.5), fancybox=True, shadow=True, ncol=1, title="Aerosol ($mg/m^2$)",
...
plt.savefig(save_dir + '/robustness/MonsoonPlanet_Hysteresis_4panels_aerosol.pdf', bbox_inches='tight')
