...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 20 09:06:41 2023

@author: anjakatzenberger
"""

# This code creates the hysteresis plots for the Monsoon Planet
# Inclung precipiation depending on solar radiation and surface temperature
# and water vapour content depending on solar radiation and surface temperature

# The code is based on daily data that is averaged over 10 years
...
for i, slab in enumerate(slab_list):
    ...
    # Mark only the first data point with a marker "."
    axs[1].plot(sw[0], wvp[0], marker=">", color=colors[i])
...
for i, slab in enumerate(slab_list):
    ...
    # Mark only the first data point with a marker "."
    axs[0].plot(sw[0], pr[0], marker=">", color=colors[i])
